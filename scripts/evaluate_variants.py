...
# Candidate name -> (model directory, decoder variant flag)
CANDIDATES = {'fp32': ('models/qwen3-asr-1.7b-fp32', 'fp32'), 'int4': ('models/qwen3-asr-1.7b-fp32', 'int4'), '0.6b': ('models/qwen3-asr-0.6b-fp32', 'fp32')}
REUSE = False

def run(variant):
    model_dir, flag = CANDIDATES[variant]
    ...
    if not (REUSE and report.exists() and 'fixtures' in json.loads(report.read_text('utf-8'))):
        proc = subprocess.run([str(exe), '--verify', str(ROOT / 'evaluation/manifest.json'), '--variant', flag, '--model', str(model), '--skip-asset-check', '--report', str(report)],
    ...
    categories = {}
    manifest = json.loads((ROOT / 'evaluation/manifest.json').read_text('utf-8'))
    ...
    differences = []
    return {'variant': variant, 'model': model_dir, 'decoder': flag, 'exit': proc.returncode, 'categories': categories, 'silence_failures': silence_failures,
...
def main():
    ...
    for r in results.values():
        r.pop('texts', None); r.pop('edits', None)
    result = {'rule': 'candidate accepted when every category increases by at most 1.0 percentage point absolute and no new non-speech fixture produces output',
